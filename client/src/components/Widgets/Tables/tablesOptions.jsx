...
class TableOptions extends Component {
  ...
  searchFieldChangeHandler = (event) => {
    ...
    if (searchByKeyword !== 0 && searchByKeyword !== "0") {
      let filteredData = this.props.state.tableData.filter((item) => {
        const searchItem = item[String(searchByKeyword)];
        if (searchItem === undefined) return false;
        if (React.isValidElement(searchItem))
          return searchItem.props.children.toLowerCase().indexOf(keyword) > -1;
        return (
          item[String(searchByKeyword)].toLowerCase().indexOf(keyword) > -1
        );
      });
  ...
  searchBar = () => {
    return (
      <div className="form-group col-md-1 col-lg-2">
        ...
        />
        {this.state.searchBy === 0 ? (
          <div id="searhBarErrorField" style={{ color: "red" }}></div>
        ) : null}
      </div>
    );
  };
    ...
    return categoryArray.map((option, i) => {
      ...
      return (
        <div key={i} className="form-group col-md-1 col-lg-2">
          <label htmlFor="filterOptionSelectBar">
            {option.field.charAt(0).toUpperCase() + option.field.substring(1)}
          </label>
  ...
  handleFilterClick = () => {
    ...
    for (let item of selectedValue) {
      ...
      tempJSON[tempJSONKey] = document.getElementById(
        `${item}-filterOptionSelectBar`
      ).value;
      ...
      return (
        <Fragment>
          <button
            ...
            onClick={(event) => {
              this.advancedButtonClickHandler(event);
            }}
          >
            {" "}
            &#8810;Advanced Search
          ...
          <div
            className="advanceSearchOptions"
  ...
  render() {
    return <div className="container-fluid">{this.tableOptions()}</div>;
  }
}
